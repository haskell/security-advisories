...
spec =
  testGroup
    ...
    [ testGroup "isAffectedBy" $
        flip concatMap cases $
          map $ \(actual, query, expected) ->
            let title x y =
                  if expected
...
cases =
  ...
  where
    reversible (query, affectedVersion, expected) = [(query, affectedVersion, expected), (query, affectedVersion, expected)]
...
mkAdvisory versionRange =
  Advisory
    { advisoryId = fromMaybe (error "Cannot mkHsecId") $ mkHsecId 2023 42,
      ...
      advisoryCWEs = [],
      advisoryKeywords = [],
      advisoryAliases = Maybe.fromJust . traverse parseOsvId $ ["CVE-2022-1234"],
      advisoryRelated = Maybe.fromJust . traverse parseOsvId $ ["CVE-2022-5678", "CVE-2022-9012"],
      advisoryAffected =
        [ Affected
  ...
  let fixed from to =
      ...
      mkMajorBoundVersion =
        \case
          ...
          [x] -> [x, 1]
          (x : y : _) -> [x, y + 1]
   in case projectVersionRange vr of

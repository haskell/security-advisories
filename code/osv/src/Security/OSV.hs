...
instance FromJSON Severity where
  parseJSON = withObject "severity" $ \o -> do
    ...
    case typ of
      ...
      "CVSS_V4" | CVSS.cvssVersion cvss == CVSS.CVSS40 -> pure $ Severity cvss
      s ->
...
instance
  ...
  ToJSON (Affected ecosystemSpecific dbSpecific rangeDbSpecific)
  where
  toJSON Affected {..} =
    object $
...
instance
  ...
  FromJSON (Affected ecosystemSpecific dbSpecific rangeDbSpecific)
  where
...
instance
  ...
  FromJSON (Model dbSpecific affectedEcosystemSpecific affectedDbSpecific rangeDbSpecific)
  where
